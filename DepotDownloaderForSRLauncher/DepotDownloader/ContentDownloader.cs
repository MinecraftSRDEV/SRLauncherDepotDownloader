...
namespace DepotDownloader
{
    ...
    static class ContentDownloader
    {
        ...
        public static async Task DownloadAppAsync(uint appId, List<(uint depotId, ulong manifestId)> depotManifestIds, string branch, string os, string arch, string language, bool lv, bool isUgc)
        {
            ...
            if (!AccountHasAccess(appId))
            {
                ...
                else
                {
                    ...
                    DepotDownloaderClient.formatAndSend("LOG", "You dont have this game on this account", 2);
        ...
        private static async Task DownloadSteam3Async(List<DepotDownloadInfo> depots)
        {
            ...
            var summarySend = new sendLogs
            {
                type = "SUMMARY",
                msg = "Total downloaded: " + downloadCounter.totalBytesCompressed + " bytes (" + downloadCounter.totalBytesUncompressed + " bytes uncompressed)",
                logType = 0
            };
            DepotDownloaderClient.sendMessage(JsonSerializer.Serialize(summarySend));
        }
            ...
            else
            {
                ...
                if (neededChunks.Count == 0)
                {
                    lock (depotDownloadCounter)
                    {
                        ...
                        var progress = (depotDownloadCounter.sizeDownloaded / (float)depotDownloadCounter.completeDownloadSize) * 100.0f;
                        DepotDownloaderClient.formatAndSend("DownloadProgress", progress.ToString(), 0);
            ...
            ProtoManifest.ChunkData chunk)
        {
            ...
            {
                var fileFinalPath = Path.Combine(depot.InstallDir, file.FileName);
                var progress = (sizeDownloaded / (float)depotDownloadCounter.completeDownloadSize) * 100.0f;
                DepotDownloaderClient.formatAndSend("DownloadProgress", progress.ToString(), 0);
