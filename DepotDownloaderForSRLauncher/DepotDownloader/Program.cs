...
namespace DepotDownloader
{
    ...
    class Program
    {
        ...
        {
            DepotDownloaderClient.connectToLauncher();
            ...
            if (args.Length == 0)
            {
                ...
                DepotDownloaderClient.formatAndSend("LOG", "Wrong arguments", 2);

                return 0;
            }

            DepotDownloaderClient.formatAndSend("LOG", "Params OK", 0);
            ...
            DepotDownloaderClient.formatAndSend("LOG", "User profile name: " + username, 0);
            ...
            if (fileList != null)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    DepotDownloaderClient.formatAndSend("LOG", "Unable to load filelist: " + ex, 1);
                }
            }
            ...
            if (pubFile != ContentDownloader.INVALID_MANIFEST_ID)
            {
                ...
                if (InitializeSteam(username, password))
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        DepotDownloaderClient.formatAndSend("LOG", "Download failed to due to an unhandled exception: " + e.Message, 2);
                        throw;
                    }
                    finally
                    {
            ...
            else
            {
                ...
                if (InitializeSteam(username, password))
                {
                    try
                    {
                        DepotDownloaderClient.formatAndSend("LOG", "Starting download", 0);
            ...
            var endlog = new sendLogs
            {
                type = "SHD",
                msg = "Process finished",
                logType = 0
            };
            DepotDownloaderClient.sendMessage(JsonSerializer.Serialize(endlog));
